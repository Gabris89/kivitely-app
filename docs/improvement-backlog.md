...
- [x] **Permission checks are hardcoded, not role-driven.** ~~`moveIssueStatusRecord` always calls `canMoveIssue(issue, targetStatus, "project_manager")` regardless of who is actually using the app.~~ Resolved 2026-07-27 (done 2026-07-27): `src/lib/currentUser.ts` resolves the signed-in user's `profiles.role` and both the server-side status move and the issue detail status dropdown now run on the real role. See `docs/permissions-plan.md` step 1.
...
- [ ] (step 2-4 of `permissions-plan.md`) Tighten insert policies before any external/public access — review the `with check (true)` policies, consider `authenticated`-only writes or basic rate limiting on the API routes.
- [x] Stop hardcoding the actor role in `canMoveIssue` calls (done 2026-07-27) — the role now comes from `profiles` via the signed-in Auth user (`src/lib/currentUser.ts`, migration `20260727090000_profiles_auth_link.sql`). Only the missing-profile fallback stays a documented placeholder (`FALLBACK_WORKFLOW_ROLE`).
